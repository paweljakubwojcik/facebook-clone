...
const Avatar = forwardRef(({ image, altText, big, large, size, ...rest }, ref) => {
    return (
        <ImageContainer
            big={big}
            large={large}
            size={size || 40}
            img={image || defaultUserImage}
            className="avatar"
            ref={ref}
            {...rest}
        />
...
const ImageContainer = styled.div`
    ...
    border-radius: 50%;
    border: solid 7px
        ${(props) => (props.large || props.big ? props.theme.primaryElementColor : 'none')};
`
